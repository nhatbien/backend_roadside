...
func NewUserRepo(sql *db.Sql) user_repo.UserRepo {
...
func (n *UserRepoImpl) UpdateLocationUser(context context.Context, UserId string, lat float64, lng float64) (model.User, error) {
	var user model.User

	if res := n.sql.Db.Where(
		&model.User{Id: UserId},
	).First(&user); res.RowsAffected <= 0 {
		return user, biedeptrai.ErrorRescueUnitNotFound
	}
	...
	err := n.sql.Db.Save(user).Error
	if err != nil {
		return user, biedeptrai.ErrorUpdateRescueUnitFail
	}
	return user, nil
}
